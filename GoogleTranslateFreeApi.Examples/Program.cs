...
namespace GoogleTranslateFreeApi.Examples
{
	class Program
	{
		...
		static void Main(string[] args)
		{
			...
			Language italian = Language.Italian; // or even this way
			Language french = GoogleTranslator.GetLanguageByISO("fr"); // you could also use ISO639 value

			TranslationResult result = Translator.TranslateAsync("Hello. How are you?", english, italian).GetAwaiter().GetResult();
